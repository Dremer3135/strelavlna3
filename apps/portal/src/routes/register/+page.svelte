<script lang="ts">
  ...
  let selectedKraj: string = $state("");
  let selectedOkres: string = $state("");
  let schools: SkolyResponse[] = $state([]);
...
<div class="auth-modal-backdrop" on:click={() => dispatch("close")}>
  <div class="auth-modal-content" on:click|stopPropagation>
      <form method="POST" use:enhance={() => {
        ...
        />
        <select bind:value={selectedKraj}
          class="school"
          on:change={() => {selectedOkres = ""}}
        >
        ...
        <select bind:value={selectedOkres}
          class="school"
          on:change={fetchSchools}
        ...
        {:else}
          <select bind:value={school}
            class="school"
            disabled={!selectedOkres || schools.length === 0}
            name="school"
          >
