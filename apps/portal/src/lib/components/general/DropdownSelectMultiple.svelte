<script lang="ts">
    ...
    type OnChangefunction = (ids: string[]) => void;

    let { selected, possible, onchangeFunction }: { selected: DropdownItem[], possible: DropdownItem[], onchangeFunction: OnChangefunction } = $props();
    ...
    <div class="content" class:open={ opened } style="--wrapper-height: { dropdownHeight }px">
        <div class="wrapper" bind:clientHeight={ dropdownHeight }>
            {#each possible as possibility }
                <button class:selected={ selected.map(item => item.id).includes(possibility.id) } onclick={() => { 
                    let current = selected.map(item => item.id);
                    if (selected.map(item => item.id).includes(possibility.id)) {
                        current = current.filter(item => item != possibility.id);
                    } else {
                        current.push(possibility.id);
                    }
